...
DECLARE 
    ...
    end_time TIMESTAMP;
BEGIN
    -- Load customer information table
    start_time := NOW();
    ...
    WHERE rk = 1; -- select the most recent record per customer
    ...
    -- Load product information table
    start_time := NOW();
    ...
    SELECT 
        ...
        CASE UPPER(TRIM(prd_line))
            ...
            WHEN 'T' THEN 'Touring'
            ELSE 'n/a'
        END AS prd_line,   -- map product line codes to descriptive
        prd_start_dt,
    ...
    SELECT 
        ...
        END AS cid,
        CASE 
            WHEN bdate < '1920-01-01' OR bdate > CURRENT_DATE THEN NULL 
            ELSE bdate
        END AS bdate,
        CASE 
    ...
    -- Load product category G1V2 table
    start_time := NOW();
